...
**Status:** Phase 0 — architecture only (no provider OAuth / meeting-creation code yet)  
**Date:** 2026-09-14  
**Audience:** product + engineering review before implementation
...
## 13. Implementation checklist (after approval — not started)

- [ ] Approve this doc + provider order
- [ ] Register Google Cloud OAuth (Calendar events) test client
- [ ] Register Zoom OAuth user-managed app (dev)
- [ ] Register Entra multi-tenant app (Teams) — can wait until Phase 3
- [ ] Migration: `teacher_provider_connections` + lesson provider columns
- [ ] `tokenCrypto` + connection service
- [ ] `LiveLessonProvider` + `MentorixLiveProvider` adapter around existing LiveKit
- [ ] `GoogleMeetProvider` + create UX picker
- [ ] Copy clarifications for Live vs BYO
- [ ] Zoom → Teams
...
**Provider OAuth and meeting-creation code is not implemented in this phase.**  
Next step: product/engineering approval → Phase 1 (Google Meet) implementation.
