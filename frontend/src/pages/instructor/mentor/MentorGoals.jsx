...
const emptyGoal = { id: null, title: '', mentee_id: '', why_text: '', success_metric: '', target_date: '', progress: 0, status: 'active' }
...
export default function MentorGoals() {
  ...
  async function saveGoal() {
    ...
    setSaving(true)
    try {
      if (form.id) await updateGoal(form.id, form)
      else await createGoal(form)
      setForm(emptyGoal)
      setGoalOpen(false)
      toast(form.id ? 'Məqsəd yeniləndi' : 'SMART məqsəd yaradıldı', 'success')
  ...
  async function toggleMilestone(goal, item) {
    try {
      const nextStatus = item.status === 'done' ? 'todo' : 'done'
      await updateMilestone(goal.id, item.id, { status: nextStatus })
      const items = goal.milestones || []
      const done = items.filter((x) => x.id === item.id ? nextStatus === 'done' : x.status === 'done').length
      const progress = items.length ? Math.round(done / items.length * 100) : Number(goal.progress || 0)
      await updateGoal(goal.id, { progress, status: progress === 100 ? 'completed' : 'active' })
    } catch (err) { toast(err?.message || 'Mərhələ yenilənmədi', 'error') }
  }

  const visibleGoals = data.goals.filter((goal) => {
    const statusMatch = filter === 'all' || goal.status === filter
    const textMatch = !query.trim() || `${goal.title} ${goal.why_text || ''} ${goal.success_metric || ''}`.toLowerCase().includes(query.trim().toLowerCase())
    return statusMatch && textMatch
  })

  return <MentorPage>
    <MentorPageHeader eyebrow="Məqsəd və inkişaf" title="Məqsədlər və yol xəritəsi" description="Hər mentorluq münasibətini 1–3 ölçülə bilən SMART məqsəd üzərində qurun. Sessiyalar və öhdəliklər bu məqsədlərə bağlanır." action={<PrimaryButton onClick={() => { setForm(emptyGoal); setGoalOpen(true) }}><MentorIcon name="plus" size={15} /> Yeni məqsəd</PrimaryButton>} />
    ...
    <MentorCard>
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-1 rounded-xl bg-slate-100 p-1">{[['active','Aktiv'],['completed','Tamamlanan'],['all','Hamısı']].map(([value,label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`rounded-lg px-3 py-2 text-[11px] font-extrabold ${filter === value ? 'bg-white text-[#087f70] shadow-sm' : 'text-slate-500'}`}>{label}</button>)}</div><input value={query} onChange={(e) => setQuery(e.target.value)} className={`${inputClass} sm:max-w-xs`} placeholder="Məqsədlərdə axtar" /></div>
      <div className="mt-5 space-y-4">
        {!loading && !visibleGoals.length && <EmptyState icon="target" title={data.goals.length ? 'Bu filtrə uyğun məqsəd yoxdur' : 'İlk inkişaf məqsədini yaradın'} text="Məsələn: 90 gün ərzində portfolio hazırlamaq və üç iş müsahibəsinə müraciət etmək." action="SMART məqsəd yarat" onAction={() => { setForm(emptyGoal); setGoalOpen(true) }} />}
        {visibleGoals.map((goal) => {
          ...
          return <article key={goal.id} className="rounded-2xl border border-slate-200 p-4 sm:p-5">
            ...
            <div className="mt-4 space-y-2">
              {(goal.milestones || []).map((item) => <label key={item.id} className="flex cursor-pointer items-center gap-3 rounded-xl px-2 py-2 hover:bg-slate-50"><input type="checkbox" checked={item.status === 'done'} onChange={() => toggleMilestone(goal, item)} className="h-4 w-4 rounded border-slate-300 accent-[#087f70]"/><span className={`flex-1 text-xs ${item.status === 'done' ? 'text-slate-400 line-through' : 'font-semibold text-slate-650'}`}>{item.title}</span><span className="text-[10px] text-slate-400">{formatDate(item.due_date, '')}</span></label>)}
              {!(goal.milestones || []).length && <p className="text-xs text-slate-400">Bu məqsəd üçün mərhələ əlavə edilməyib.</p>}
            </div>
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4"><div className="flex gap-3"><button type="button" onClick={() => { setMilestoneGoal(goal); setMilestone({ title: '', due_date: '' }) }} className="inline-flex items-center gap-1 text-xs font-extrabold text-[#087f70]"><MentorIcon name="plus" size={14} /> Mərhələ əlavə et</button><button type="button" onClick={() => { setForm({ id: goal.id, title: goal.title, mentee_id: goal.mentee_id || '', why_text: goal.why_text || '', success_metric: goal.success_metric || '', target_date: goal.target_date ? String(goal.target_date).slice(0, 10) : '', progress: goal.progress, status: goal.status }); setGoalOpen(true) }} className="inline-flex items-center gap-1 text-xs font-bold text-slate-500"><MentorIcon name="edit" size={13} /> Redaktə et</button></div><div className="flex items-center gap-2"><span className="text-[10px] text-slate-400">{done}/{(goal.milestones || []).length} addım</span><select value={goal.progress} onChange={(event) => setProgress(goal, Number(event.target.value))} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px] font-bold text-slate-600 outline-none"><option value="0">0%</option><option value="25">25%</option><option value="50">50%</option><option value="75">75%</option><option value="100">100%</option></select></div></div>
          </article>
        })}
      </div>
    </MentorCard>

    <MentorModal open={goalOpen} title={form.id ? 'Məqsədi redaktə et' : 'Yeni SMART məqsəd'} description="Bir nəticə seçin və onu ölçülə bilən edin." onClose={() => setGoalOpen(false)} footer={<><SecondaryButton onClick={() => setGoalOpen(false)}>Ləğv et</SecondaryButton><PrimaryButton disabled={saving} onClick={saveGoal}>{form.id ? 'Yenilə' : 'Məqsədi yarat'}</PrimaryButton></>}>
      <div className="space-y-4"><Field label="Məqsəd"><input className={inputClass} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="məsələn, Data analyst portfolio-su hazırlamaq" /></Field><Field label="Mentee"><select className={inputClass} value={form.mentee_id} onChange={(e) => setForm({ ...form, mentee_id: e.target.value })}><option value="">Mentee seçilməyib</option>{data.mentees.map((item) => <option key={item.id} value={item.id}>{menteeName(item)}</option>)}</select></Field><Field label="Bu niyə vacibdir?"><textarea rows="3" className={inputClass} value={form.why_text} onChange={(e) => setForm({ ...form, why_text: e.target.value })} placeholder="Mentee üçün şəxsi və ya peşəkar dəyəri" /></Field><Field label="Uğur necə ölçüləcək?"><input className={inputClass} value={form.success_metric} onChange={(e) => setForm({ ...form, success_metric: e.target.value })} placeholder="məsələn, 3 layihə və 5 müraciət" /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Hədəf tarixi"><input type="date" className={inputClass} value={form.target_date} onChange={(e) => setForm({ ...form, target_date: e.target.value })} /></Field><Field label="Status"><select className={inputClass} value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="active">Aktiv</option><option value="paused">Dayandırılıb</option><option value="completed">Tamamlanıb</option></select></Field></div></div>
    </MentorModal>
