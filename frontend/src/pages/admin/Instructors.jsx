...
export default function AdminInstructors() {
  ...
  return (
    <div className="p-6">
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Yeni Müəllim Əlavə Et">
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <Button onClick={addInstructor} loading={loading} className="flex-1 justify-center">Əlavə Et</Button>
      ...
      {/* Limits Modal */}
      <Modal open={limitsModal} onClose={() => setLimitsModal(false)} title={`${selected?.full_name} — Limitlər`}>
        <div className="space-y-4">
          {[
            { key: 'sms_limit', label: 'Aylıq SMS Limiti', unit: 'SMS' },
            { key: 'storage_limit_mb', label: 'Storage Limiti', unit: 'MB' },
            { key: 'ram_limit_mb', label: 'RAM Limiti', unit: 'MB' },
            { key: 'max_concurrent_students', label: 'Maks. Eyni Vaxt Tələbə', unit: 'nəfər' },
          ].map(({ key, label, unit }) => (
            <div key={key}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button variant="secondary" onClick={() => setLimitsModal(false)} className="flex-1 justify-center">Ləğv et</Button>
          </div>
        </div>
      </Modal>
    </div>
