...
const ROLES = [
  { key: 'instructor', label: 'Müəllim', emoji: '👨‍🏫', otp: false },
...
export default function Login() {
  ...
  const [role, setRole] = useState(isAdmin ? 'admin' : null)
  ...
  const isOtp = selected?.otp || false
        ...
        <div className="bg-[#1a1740] border border-indigo-500/20 rounded-2xl p-8">
          ...
          {isAdmin && (
            <form onSubmit={handleEmailLogin} className="space-y-4">
              <div className="text-xs text-center text-gray-500 mb-4">Admin Panel</div>
              <div>
                <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">E-poçt</label>
                <input className="w-full bg-[#13112e] border border-indigo-500/20 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-blue-500 transition-colors"
                  type="email" placeholder="admin@mentorix.biz"
          ...
          {/* Normal mode - role selection */}
          {!isAdmin && (
            <>
              {/* Role buttons */}
              <div className="grid grid-cols-3 gap-3 mb-6">
                {ROLES.map((r) => (
                  <button key={r.key} onClick={() => { setRole(r.key); setOtpSent(false) }}
                    className={`p-3 rounded-xl border text-sm font-semibold transition-all flex flex-col items-center gap-1 ${
                      role === r.key
                        ? 'border-blue-500 bg-blue-500/20 text-blue-300'
                        : 'border-indigo-500/20 text-gray-400 hover:border-indigo-500/40 hover:text-white'
                    }`}>
              ...
              {role === 'instructor' && (
                <form onSubmit={handleEmailLogin} className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">E-poçt</label>
                    <input className="w-full bg-[#13112e] border border-indigo-500/20 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-blue-500 transition-colors"
                      type="email" placeholder="muellim@email.com"
                      value={email} onChange={(e) => setEmail(e.target.value)} required />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Şifrə</label>
                ...
                <div className="space-y-4">
                  {!otpSent ? (
                    ...
                    <form onSubmit={handleVerifyOtp} className="space-y-4">
                      <div className="text-center text-xs text-gray-400">{phone} nömrəsinə kod göndərildi</div>
                      <input className="w-full bg-[#13112e] border border-indigo-500/20 rounded-xl px-4 py-4 text-white text-2xl font-bold text-center tracking-widest outline-none focus:border-blue-500 transition-colors"
