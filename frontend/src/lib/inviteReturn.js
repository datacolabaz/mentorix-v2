...
/** Admin-only return path-ləri non-admin üçün ev panelinə yönəlt; partner next admin-i kabinetə məcbur etməsin. */
export function isAllowedReturnPathForUser(user, path) {
  ...
  // Partner kabineti admin home-u override etməsin (logout sticky next).
  if (p.startsWith('/partner')) return role !== 'admin'
  return true
}
