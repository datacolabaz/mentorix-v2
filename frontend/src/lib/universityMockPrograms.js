import { fieldSearchTerms as fieldSearchTermsFromCatalog, FIELD_GROUPS, relatedFieldSlugs } from './universityFieldCatalog'

const FIELD_BY_VALUE = new Map(FIELD_GROUPS.flatMap((g) => g.options.map((o) => [o.value, { ...o, category: g.id }])))
function fieldMeta(slug) { return FIELD_BY_VALUE.get(String(slug || '').trim()) || null }
function fieldSearchTerms(slug) { return fieldSearchTermsFromCatalog(slug) }
...
function filterMockPrograms(filters = {}) {
  let rows = [...MOCK_PROGRAMS];

  if (filters.degreeLevel) {
    rows = rows.filter((p) => p.degree_level === filters.degreeLevel);
  }
  if (filters.field) {
    rows = rows.filter((p) => programMatchesField(p, filters.field));
  }
