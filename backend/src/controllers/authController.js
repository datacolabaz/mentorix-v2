...
const login = async (req, res) => {
  try {
    ...
    if (!user || !(await bcrypt.compare(password, user.password_hash)))
      return res.status(401).json({ success: false, message: 'Email ve ya sifre yanlisdir' });
    if (user.role !== 'admin')
      return res.status(403).json({ success: false, message: 'Yalniz admin email ile daxil ola biler' });
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const clean = phone.replace(/\D/g, '');
    const { rows } = await db.query(
      'SELECT * FROM otp_codes WHERE phone = $1 AND code = $2 AND is_used = FALSE AND expires_at > NOW()', [clean, code]);
    if (!rows[0]) return res.status(400).json({ success: false, message: 'Kod yanlis ve ya muddeti bitib' });
...
const register = async (req, res) => {
  try {
    ...
    const result = await db.transaction(async (client) => {
      ...
      const user = rows[0];
      if (role === 'instructor') {
        await client.query('INSERT INTO instructor_profiles (user_id, subject, billing_type) VALUES ($1, $2, $3)',
          [user.id, subject || null, billing_type || '8_lessons']);
      } else if (role === 'student') {
        await client.query('INSERT INTO student_profiles (user_id, parent_id) VALUES ($1, $2)', [user.id, parent_id || null]);
      }
      return user;
    });
