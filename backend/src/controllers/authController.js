...
const setPin = async (req, res) => {
  try {
    const { pin } = req.body;
    if (!pin || pin.length !== 6 || isNaN(pin)) return res.status(400).json({ success: false, message: "6 reqemli PIN" });
    const hash = await bcrypt.hash(pin, 12);
    await db.query("UPDATE users SET pin_hash = $1 WHERE id = $2", [hash, req.user.id]);
...
const loginWithPin = async (req, res) => {
  try {
    const { phone, pin } = req.body;
    const clean = phone.replace(/\D/g, "");
    const { rows } = await db.query("SELECT * FROM users WHERE REPLACE(REPLACE(phone,\"+\",\"\"),\"-\",\"\") = $1 AND is_active = TRUE", [clean]);
    const user = rows[0];
    if (!user) return res.status(404).json({ success: false, message: "Tapilmadi" });
...
module.exports = { login, setPin, loginWithPin, sendOtp, verifyOtp, register, me, setPin, loginWithPin };
