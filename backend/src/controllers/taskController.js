...
const listInstructorGroups = async (req, res) => {
  try {
    // Same instructor-id matching as /instructor/teaching (JWT UUID dash/case variants).
    // Only real CRM teaching groups — never Link-iştirakçıları / exam / assignment cohorts.
    const instructorIdNorm = String(req.user.id || '')
      .trim()
      .toLowerCase()
      .replace(/-/g, '');
    const { rows } = await db.query(
