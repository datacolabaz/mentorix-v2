...
function availableActions(role, pathname) {
...
function matchFaq(question, locale) {
  const q = normalizeText(question);
  if (!q) return null;
  let best = null;
  let bestScore = 0;
  for (const item of FAQ) {
    const hay = normalizeText([item.q.az, item.q.ru, item.q.en].join(' '));
    const keywords = hay.split(' ').filter((k) => k.length > 2);
  ...
  return loc(best.a, locale);
}

function unknownAnswer(locale) {
  if (locale === 'ru') {
    return 'Этой функции в Mentorix нет. Я объясняю только существующие страницы: панель, поиск учителей, расписание, экзамены, настройки и админку.';
  }
  return 'Bu funksiya Mentorix-də yoxdur. Yalnız mövcud səhifələri izah edirəm: panel, müəllim axtarışı, cədvəl, imtahan, tənzimləmələr və admin.';
}

function pageFallback(context, locale) {
  if (context?.pageSummary) {
    return context.pageSummary;
  }
